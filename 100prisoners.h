 ...
 * best strategy and returns success or failure.
...
/*
 * Simulates 100 prisoners problem "n" times using threads.
 * Each thread does (n / numThreads) simulations, then stores the
 * number of successes of each simulation in an array (called successes).
 * simulateAndStatsWithThreads then prints the statistics when all threads
 * finish simulating.
 *
 * int n is the total number of simulations to be performed
 *
 * int numThreads is the number of threads to create and simulate the
 * 100 prisoners problem (n / numThreads) times.
 *
 * eg. if n == 100, and numThreads == 4, then each thread performes 100/4 = 25 simulations
 */
